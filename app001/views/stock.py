...
def stock(request):
    ...
    queryset = models.rawstock.objects.all().order_by('-id')
    form = rawstock_Mforms
...
def stockadd(request):
    """新建原材料（AJAX）"""
    form = rawstock_Mforms(data=request.POST)
    if form.is_valid():
...
def stock_detail(request):
    ...
    row_dict = models.rawstock.objects.filter(id=stockid).values('name', 'remark', 'number', 'amount',
                                                                 'price').first()
    if not row_dict:
...
def stock_edit(request):
    ...
    # return render(request, 'stock_edit.html', {"form": form})
    stockid = request.GET.get("stockid")
    row_object = models.rawstock.objects.filter(id=stockid).first()
    if not row_object:
...
def stock_delete(request):
    ...
    # return redirect('/stock/')
    stockid = request.GET.get('stockid')
    exists = models.rawstock.objects.filter(id=stockid).exists()
    if not exists:
        return JsonResponse({"status": False, 'error': "删除失败"})
    models.rawstock.objects.filter(id=stockid).delete()
    return JsonResponse({"status": True})
